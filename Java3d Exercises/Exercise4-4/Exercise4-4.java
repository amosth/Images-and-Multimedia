 ...
 * MATRICOLA:	134059
 */
public class Ex44 extends Applet {
	...
	public static void setViewPoint(SimpleUniverse simpleU, Canvas3D canvas3d, BranchGroup scene, boolean compatMode) {
		...
		viewTransform.lookAt(new Point3d(0, 7.5, distance*40),
							 new Point3d(0, 7.5, 0),
							 new Vector3d(0, 1, 0));
		...
		*/
		...
		//Creazione del behaviour per navigazione da tastiera
		KeyNavigatorBehavior keyNavBeh = new KeyNavigatorBehavior(tg);
		keyNavBeh.setSchedulingBounds(new BoundingSphere(new Point3d(), 500.0));
		scene.addChild(keyNavBeh);
		
		//Impostazione distanza dal piano frontale e dal piano posteriore
		view.setFrontClipDistance(0.1);
		view.setBackClipDistance(130.0);
	...
	public BranchGroup createSceneGraph() {
		...
		myMouseRotate.setSchedulingBounds(new BoundingSphere(new Point3d(0.d,0.d,0.d),500.0d));
	...
	public TransformGroup createSubGraph() {
		...
		Temple tempio = new Temple();
		transform.addChild(tempio);
	...
	private void lightDefinition(BranchGroup bg) {
		...
		BoundingSphere bounds = new BoundingSphere(new Point3d(0.d,0.d,0.d), 1000.0d);
	...
	public static void main(String[] args) {
		new MainFrame(new Ex44(), 1024, 768);
	}
}
